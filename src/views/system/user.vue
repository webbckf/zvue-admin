    ...
    <el-container>
      <el-row :gutter="0">
        <el-col :span="5">
          <el-aside width="100%">
             <basic-container>
                <!-- <avue-tree :option="treeOption" :data="init.deptTree" @node-click="nodeClick"></avue-tree> -->
                <ZvueSearchTree :data="init.deptTree" @node-click="nodeClick"></ZvueSearchTree>
             </basic-container>
          </el-aside>
        </el-col>
        <el-col :span="19">
...
import ZvueSearchTree from '@/components/tree/zvue-search-tree'
export default {
  components: {
    ZvueSearchTree
  },
  ...
  methods: {
    ...
    onLoad (page, params = {}) {
      ...
      getDeptTree(this.form.tenantId).then(res => {
        const index = this.$refs.crud.findColumnIndex("deptId");
        this.option.column[index].dicData = res.data.data;
        this.init.deptTree = res.data.data;
      });
      getRoleTree(this.form.tenantId).then(res => {
        const index = this.$refs.crud.findColumnIndex("roleId");
        this.option.column[index].dicData = res.data.data;
